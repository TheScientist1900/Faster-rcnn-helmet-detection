...
class FRCNNDataset(Dataset):
    def __init__(self, cfg, mode='train'):
        with open(os.path.join(cfg.data.root, '{}.txt'.format(mode)), 'r') as f:
            self.annotation_lines = f.readlines()
        self.input_shape = cfg.data.input_shape
        self.mode = mode
        self.length = len(self.annotation_lines)

        logger.info('=> {} dataset loads {} samples'.format(mode, self.length))
    def __len__(self):
        return self.length
    ...
    def __getitem__(self, index):
        ...
        image, y = self.get_random_data(
            self.annotation_lines[index], self.input_shape[0:2], random=self.mode == 'train')
