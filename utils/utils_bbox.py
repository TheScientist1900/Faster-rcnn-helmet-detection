...
def loc2bbox(src_bbox, loc):
    '''
        src_bbox: h*w, 4,  x1,y1,x2,y2
        loc: h*w, 4
    '''
    if src_bbox.size()[0] == 0:
        return torch.zeros((0, 4), dtype=loc.dtype)

    src_width = torch.unsqueeze(src_bbox[:, 2] - src_bbox[:, 0], -1)  # h*w, 1
    src_height = torch.unsqueeze(src_bbox[:, 3] - src_bbox[:, 1], -1)  # h*w, 1
    src_ctr_x = torch.unsqueeze(src_bbox[:, 0], -1) + 0.5 * src_width  # h*w, 1
    src_ctr_y = torch.unsqueeze(
        src_bbox[:, 1], -1) + 0.5 * src_height    # h*w, 1
    ...
    ctr_x = dx * src_width + src_ctr_x  # h*w, 1
...
class DecodeBox():
    ...
    def forward(self, roi_cls_locs, roi_scores, rois, image_shape, input_shape, nms_iou=0.3, confidence=0.5):
        ...
        for i in range(batch_size):
            ...
            #   第三维度是对应种类的调整参数
            #----------------------------------------------------------#
            roi_cls_loc = roi_cls_loc.view(
                [-1, self.num_classes, 4])   # 300, num_classes, 4
            ...
            cls_bbox = loc2bbox(roi.contiguous().view(
                (-1, 4)), roi_cls_loc.contiguous().view((-1, 4)))
            # 300, num_classes, 4
            ...
            prob = F.softmax(roi_score, dim=-1)  # 300, num_classes
            ...
            for c in range(1, self.num_classes):
                ...
                if len(c_confs[c_confs_m]) > 0:
                    ...
                    #   取出得分高于confidence的框
                    #-----------------------------------------#
                    boxes_to_process = cls_bbox[c_confs_m, c]  # m, 4
                    confs_to_process = c_confs[c_confs_m]   # m
                    ...
                    #   取出在非极大抑制中效果较好的内容
                    #-----------------------------------------#
                    good_boxes = boxes_to_process[keep]    # nms, 4
                    confs = confs_to_process[keep][:, None]   # nms, 1
                    # nms, 1
                    labels = (c - 1) * torch.ones((len(keep), 1)
                                                  ).cuda() if confs.is_cuda else (c - 1) * torch.ones((len(keep), 1))
                    #-----------------------------------------#
                    #   将label、置信度、框的位置进行堆叠。
                    #-----------------------------------------#
                    c_pred = torch.cat(
                        (good_boxes, confs, labels), dim=1).cpu().numpy()   # nms, 6
                    # 添加进result里
            ...
            if len(results[-1]) > 0:
                ...
                results[-1][:, :4] = self.frcnn_correct_boxes(
