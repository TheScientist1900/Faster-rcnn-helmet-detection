...
def bbox_iou(bbox_a, bbox_b):
    ...
    tl = np.maximum(bbox_a[:, None, :2], bbox_b[:, :2]
                    )  # a, b, 2: bbox_a每一个item与所有的bbox_b相比大的p1
    # a, b, 2: bbox_a每一个item与所有的bbox_b相比小的p1
    br = np.minimum(bbox_a[:, None, 2:], bbox_b[:, 2:])
    area_i = np.prod(br - tl, axis=2) * (tl < br).all(axis=2)   # a, b
    area_a = np.prod(bbox_a[:, 2:] - bbox_a[:, :2], axis=1)  # a,
    area_b = np.prod(bbox_b[:, 2:] - bbox_b[:, :2], axis=1)
    return area_i / (area_a[:, None] + area_b - area_i)  # a, b
...
class AnchorTargetCreator(object):
    def __init__(self, cfg):
        self.n_sample = cfg.train.anchor_target_creator.n_sample
        self.pos_iou_thresh = cfg.train.anchor_target_creator.pos_iou_thresh
        self.neg_iou_thresh = cfg.train.anchor_target_creator.neg_iou_thresh
        self.pos_ratio = cfg.train.anchor_target_creator.pos_ratio
    ...
    def _calc_ious(self, anchor, bbox):
        ...
        for i in range(len(gt_argmax_ious)):
            ...
            # if argmax_ious[gt_argmax_ious[i]] != i:
            #     print('不等于')
    ...
    def _create_label(self, anchor, bbox):
        ...
        pos_index = np.argwhere(label == 1).ravel()  # note
        ...
        neg_index = np.argwhere(label == 0).ravel()  # note
...
class ProposalTargetCreator(object):
    def __init__(self, cfg):
        self.n_sample = cfg.train.proposal_target_creator.n_sample
        self.pos_ratio = cfg.train.proposal_target_creator.pos_ratio
        self.pos_roi_per_image = np.round(self.n_sample * self.pos_ratio)
        self.pos_iou_thresh = cfg.train.proposal_target_creator.pos_iou_thresh
        self.neg_iou_thresh_high = cfg.train.proposal_target_creator.neg_iou_thresh_high
        self.neg_iou_thresh_low = cfg.train.proposal_target_creator.neg_iou_thresh_low
    ...
    def __call__(self, roi, bbox, label, loc_normalize_std=(0.1, 0.1, 0.2, 0.2)):
        ...
        #----------------------------------------------------------------#
        pos_index = np.argwhere(max_iou >= self.pos_iou_thresh).ravel()  # note
        pos_roi_per_this_image = int(
            min(self.pos_roi_per_image, pos_index.size))
        if pos_index.size > 0:
        ...
        #-----------------------------------------------------------------------------------------------------#
        neg_index = np.argwhere((max_iou < self.neg_iou_thresh_high) & (
            max_iou >= self.neg_iou_thresh_low)).ravel()
...
class FasterRCNNTrainer(nn.Module):
    def __init__(self, cfg, faster_rcnn, optimizer):
        ...
        self.anchor_target_creator = AnchorTargetCreator(cfg)
        self.proposal_target_creator = ProposalTargetCreator(cfg)

        self.loc_normalize_std = cfg.train.proposal_target_creator.loc_normalize_std
    ...
    def forward(self, imgs, bboxes, labels, scale):
        ...
        #   获取公用特征层
        #-------------------------------#
        base_feature = self.faster_rcnn.extractor(imgs)  # n, 2048, h, w
        ...
        for i in range(n):
            bbox = bboxes[i]  # b, 4
            label = labels[i]  # b,
            rpn_loc = rpn_locs[i]   # h*w, 4*9
            rpn_score = rpn_scores[i]  # h*w, 2*9
            roi = rois[roi_indices == i]    # 600,4
            feature = base_feature[i]   # 2048, h, w
            ...
            # ------------------------------------------------------ #
            #   roi_cls_loc: 1, n_sample, n_classes*4
            #   roi_score: 1, n_sample, n_classes
            # ------------------------------------------------------ #
            ...
            #   roi_cls_loc: n_sample, n_classes, 4
            #   roi_score: 1, n_sample, n_classes
            #   roi_loc: n_sample, 4
            # ------------------------------------------------------ #
        ...
        losses = [rpn_loc_loss_all/n, rpn_cls_loss_all /
                  n, roi_loc_loss_all/n, roi_cls_loss_all/n]
